...
class Hierarchical extends JFrame implements GLEventListener, KeyListener, MouseListener, MouseMotionListener, ActionListener {
	...
	private objModel fan = new objModel("Objects/ceiling_fan.obj");
	private objModel bunny = new objModel("Objects/bunny.obj");
	private objModel bunny2 = new objModel("Objects/bunny2.obj");
	private objModel armadillo = new objModel("Objects/armadillo.obj");
	private objModel male = new objModel("Objects/male.obj");
	private objModel shoe = new objModel("Objects/shoe.obj");
	private objModel shoe2 = new objModel("Objects/shoe.obj");
	private objModel vase = new objModel("Objects/vase.obj");
	private objModel tulip = new objModel("Objects/tulip.obj");

	private float fan_rotate = 0.f;
	private float bunny2_rotate = 0.f;
	private float shoe_rotate = 0.f;
	private float tulip_rotate = 0.f;
	private float male_rotate = 0.f;
	private float vase_rotate = 0.f;
	private float armadillo_rotate = 0.f;
	...
	public void display(GLAutoDrawable drawable) {
		...
		gl.glPushMatrix();
			...
			gl.glTranslatef(fan.center.x, fan.center.y+1.0f, fan.center.z);		
			gl.glRotatef(fan_rotate, 0, 1, 0);
			fan.Draw();
			gl.glPushMatrix();
				gl.glTranslatef(-0.5f, -.5f, 0);
				gl.glScalef(0.1f, 0.1f, 0.1f);
				gl.glRotatef(0, 0, 1, 0);
				bunny.Draw();
				gl.glPushMatrix();
					gl.glRotatef(bunny2_rotate, 1, 1, 1 );
					gl.glTranslatef(0.5f, .5f, -.5f);
					gl.glScalef(0.65f, 0.65f, 0.65f);
					bunny2.Draw();
					gl.glPushMatrix();
						gl.glColor3f(0.9f,0.6f,0.3f);
						gl.glTranslatef(1f, 1f, 1f);
						gl.glScalef(2.0f, 2.0f, 2.0f);
						gl.glRotatef(armadillo_rotate, 1, 0, 0);
						armadillo.Draw();
					gl.glPopMatrix();
				gl.glPopMatrix();
			gl.glPopMatrix();
			gl.glPushMatrix();
				gl.glColor3f(0.75f,0.62f,0.5f);
				gl.glRotatef(male_rotate, 0, 1, 1 );
				gl.glTranslatef(0.2f, -.8f, -.3f);
				gl.glScalef(8.0f, 8.0f, 8.0f);
				gl.glScalef(0.1f, 0.1f, 0.1f);
				male.Draw();
				gl.glPushMatrix();
					gl.glColor3f(0.5f,0.5f,0.5f);
					gl.glRotatef(-shoe_rotate, 0, 1, 0 );
					gl.glTranslatef(.1f, .3f, -.2f);
					gl.glRotatef(-shoe_rotate, 1, 0, 0 );
					gl.glScalef(0.1f, 0.1f, 0.1f);
					shoe.Draw();
				gl.glPopMatrix();
				gl.glPushMatrix();
			...
			gl.glPushMatrix();
				...
				gl.glTranslatef(-.3f, -1.1f, .4f);
				gl.glRotatef(vase_rotate, 1, 1, 1 );
				gl.glScalef(0.2f, 0.2f, 0.2f);	
				vase.Draw();
				gl.glPushMatrix();
					gl.glColor3f(0.5f,0f,0.5f);
					gl.glRotatef(tulip_rotate, 0, 1, 0 );
					gl.glTranslatef(-.3f, .7f, .3f);
					gl.glRotatef(tulip_rotate, 1, 0, 0 );
		...
		if (animator.isAnimating()){
			fan_rotate += -1.0f * animation_speed;
			shoe_rotate += 1.5f * animation_speed;
			tulip_rotate += 1.0f * animation_speed;
			male_rotate += 0.5f * animation_speed;
			vase_rotate += -1.0f * animation_speed;
			bunny2_rotate += -1.5f * animation_speed;
			armadillo_rotate += -2f * animation_speed;
		}
	}
	
	public void setColour(float[] mat_specular, float[] mat_diffuse){
